...
func Pipeifangfa(canshu string, canshuleixing string, muluming string, wenjianming string) []string {
	...
	// func \(sjk \*Sjk\)
	// func \(zfzhi \*Zfzhi\)
	repstr := zf.Zfs.Func(true) + zfzhi.Zhi.Kgf() + zfzhi.Zhi.Fxx() + zfzhi.Zhi.Xkhz() + canshu + zfzhi.Zhi.Kgf() + zfzhi.Zhi.Fxx() + zfzhi.Zhi.Xh() + canshuleixing + zfzhi.Zhi.Fxx() + zfzhi.Zhi.Xkhy() + zfzhi.Zhi.Kgf()
	//拼接从文件中匹配方法名的正则表达式
	// func \(zf \*Zf\) [[:word:]]+
	// func \(sjk \*Sjk\) [[:word:]]+
	// func \(zfzhi \*Zfzhi\) [[:word:]]+
	regstr := repstr + zfzhi.Zhi.Zkhz() + zfzhi.Zhi.Zkhz() + zfzhi.Zhi.Mh() + zf.Zfs.Word(true) + zfzhi.Zhi.Mh() + zfzhi.Zhi.Zkhy() + zfzhi.Zhi.Zkhy() + zfzhi.Zhi.Jia()
	//生成正则表达式
